...
def getmask(length, max_len, out_shape):
    ''' 
    length shape:[batch_size]    true length 30
    '''
...
def softmax_by_length(inputs, length):
    ...
    '''
    inputs = tf.exp(tf.cast(inputs, tf.float32))    #e的0次幂是1
    inputs *= getmask(length, tf.shape(inputs)[2], tf.shape(inputs))   #30  返回一个（？，1，30）tensor 1 或者 0
    _sum = tf.reduce_sum(inputs, reduction_indices=2, keepdims =True) + 1e-9
    return inputs / _sum               #(?,1,30)

def att_var(inputs,length,w1,b1,w2):
    ''' 
    input shape:[batch_size, max_len, n_hidden]
    length shape:[batch_size]  batch size个
    return shape:[batch_size, n_hidden]
    '''
    max_len, n_hidden = (tf.shape(inputs)[1], tf.shape(inputs)[2])   #30  200
    print(max_len,n_hidden)
    tmp = tf.reshape(inputs, [-1, n_hidden])   #200  长度batchsize个
    u = tf.tanh(tf.matmul(tmp, w1) + b1)        #每个单词进行全连接（？，200）
    alpha = tf.reshape(tf.matmul(u, w2), [-1, 1, max_len])   #(?,1) -> (?,1,30)
    alpha = softmax_by_length(alpha, length)                    #(?,1,30)
    return tf.reshape(tf.matmul(alpha, inputs), [-1, n_hidden])     #(?,30.200)  ->(?,1,200)->(?,200)
